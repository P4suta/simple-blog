-- Typography for the scripts the site may be written in (ruby readings,
-- language-specific families, no mid-word breaks in Latin), figures for the
-- images the release now wraps, a related-posts section, a visible keyboard
-- focus, and the copy button the article script adds to code blocks. As
-- always, only stylesheets still equal to the previous default are updated.
